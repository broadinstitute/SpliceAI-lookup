...
DISABLE_LOGGING_FOR_IPS = {
    f"63.143.42.{i}" for i in range(0, 256)
}  # ignore uptimerobot.com IPs
...
GENCODE_VERSION = "v49"
SPLICEAI_GRCH37_ANNOTATIONS = (
    f"./annotations/gencode.{GENCODE_VERSION}lift37.basic.annotation.txt.gz"
)
SPLICEAI_GRCH38_ANNOTATIONS = (
    f"./annotations/gencode.{GENCODE_VERSION}.basic.annotation.txt.gz"
)
PANGOLIN_GRCH37_ANNOTATIONS = f"./annotations/gencode.{GENCODE_VERSION}lift37.basic.annotation.without_chr_prefix.db"
PANGOLIN_GRCH38_ANNOTATIONS = (
    f"./annotations/gencode.{GENCODE_VERSION}.basic.annotation.db"
)
...
for genome_version, annotation_path in ("37", SPLICEAI_GRCH37_ANNOTATIONS), (
    "38",
    SPLICEAI_GRCH38_ANNOTATIONS,
):
    ...
    for _, row in df.iterrows():
        ...
        ANNOTATION_INTERVAL_TREES[genome_version][chrom].add(
            Interval(row["TX_START"], row["TX_END"] + 0.1, row["#NAME"])
        )
...
for genome_version in "37", "38":
    ...
    df = pd.read_table(
        ...
        else SPLICEAI_GRCH38_ANNOTATIONS
    )
    spliceai_annotation_transcript_ids = set(
        df["#NAME"].apply(lambda t: t.split(".")[0])
    )
    transcript_ids_without_annotations = (
        spliceai_annotation_transcript_ids - json_transcript_ids
    )
    if len(transcript_ids_without_annotations) > 0:
        raise ValueError(
            f"Missing {len(transcript_ids_without_annotations)} transcripts in {genome_version} annotations: {transcript_ids_without_annotations}"
        )
...
if USE_REDIS:
    ...
    )  # in-memory cache server which may or may not be running
else:
    REDIS = None
...
def get_splicing_scores_from_redis(tool_name, variant, genome_version, distance, mask):
    ...
    key = get_splicing_scores_redis_key(
        tool_name, variant, genome_version, distance, mask
    )
    results = None
    try:
...
def add_splicing_scores_to_redis(
    tool_name, variant, genome_version, distance, mask, results
):
    if REDIS is None:
        return
...
def exceeds_rate_limit(user_id, request_type):
    ...
    if (
        epoch_time - int(REDIS.get("rate_limit_outlier_ips_update_time") or 0) > 120
    ):  # time 2 minutes
    ...
    try:
        ...
        if len(keys) >= max_requests:
            ...
            REDIS.expire(
                redis_hit_limit_counter_key,
                RATE_LIMIT_COUNTER_WINDOW_SIZE_IN_DAYS * 24 * 60 * 60,
            )
        ...
        REDIS.expire(
            f"{redis_key_prefix}: {epoch_time}", RATE_LIMIT_WINDOW_SIZE_IN_MINUTES * 60
        )
    except Exception as e:
        print(f"Redis error: {e}", flush=True)
...
def get_spliceai_scores(variant, genome_version, distance_param, mask_param):
    ...
    if not ANNOTATION_INTERVAL_TREES[genome_version][chrom_without_chr].at(pos):
        ...
        other_genome_overlapping_intervals = ANNOTATION_INTERVAL_TREES[
            other_genome_version
        ][chrom_without_chr].at(pos)
        if other_genome_overlapping_intervals:
            other_genome_genes = " and ".join(
                sorted(set([str(i.data) for i in other_genome_overlapping_intervals]))
            )
            return {
                "variant": variant,
                "source": "spliceai",
    ...
    if not scores:
        error_message = exceeds_rate_limit(
            request.remote_addr, request_type="spliceai:model"
        )
        if error_message:
            return {
                "variant": variant,
        ...
        try:
            scores = get_delta_scores(
                record, SPLICEAI_ANNOTATOR[genome_version], distance_param, mask_param
            )
    ...
    for i, transcript_scores in enumerate(scores):
        ...
        transcript_annotations = TRANSCRIPT_ANNOTATIONS[genome_version].get(
            transcript_id_without_version
        )
        if transcript_annotations is None:
            raise ValueError(
                f"Missing annotations for {transcript_id_without_version} in {genome_version} annotations"
            )
        ...
        current_transcript_priority = TRANSCRIPT_PRIORITY_ORDER[
            transcript_annotations["t_priority"]
        ]
        current_delta_score_sum = sum(
            float(transcript_scores.get(key, 0))
        ...
        if current_transcript_priority > all_non_zero_scores_transcript_priority:
            ...
            current_transcript_priority == all_non_zero_scores_transcript_priority
            and current_delta_score_sum > max_delta_score_sum
        ):
...
def get_pangolin_scores(variant, genome_version, distance_param, mask_param):
    ...
    error_message = exceeds_rate_limit(
        request.remote_addr, request_type="pangolin:model"
    )
    if error_message:
        return {
            "variant": variant,
    ...
    for i, transcript_scores in enumerate(scores):
        ...
        current_delta_score_sum = sum(
            abs(float(s.get("SG_ALT", 0)) - float(s.get("SG_REF", 0)))
            for s in transcript_scores["ALL_NON_ZERO_SCORES"]
        )
        current_delta_score_sum += sum(
            abs(float(s.get("SL_ALT", 0)) - float(s.get("SL_REF", 0)))
            for s in transcript_scores["ALL_NON_ZERO_SCORES"]
        )
...
def run_splice_prediction_tool(tool_name):
    ...
    error_message = exceeds_rate_limit(
        request.remote_addr, request_type=f"{tool_name}:total"
    )
    if error_message:
        print(
            f"{logging_prefix}: {request.remote_addr}: response: {error_message}",
            flush=True,
        )
    ...
    if not variant:
        return error_response(
            f'"variant" not specified. For example: {SPLICEAI_EXAMPLE}\n',
            source=tool_name,
        )

    if not isinstance(variant, str):
        return error_response(
            f'"variant" value must be a string rather than a {type(variant)}.\n',
            source=tool_name,
        )
    ...
    if not genome_version:
        return error_response(
            f'"hg" not specified. The URL must include an "hg" arg: hg=37 or hg=38. For example: {SPLICEAI_EXAMPLE}\n',
            source=tool_name,
        )

    if genome_version not in ("37", "38"):
        return error_response(
            f'Invalid "hg" value: "{genome_version}". The value must be either "37" or "38". For example: {SPLICEAI_EXAMPLE}\n',
            source=tool_name,
        )
    ...
    try:
        ...
        return error_response(
            f'Invalid "distance": "{distance_param}". The value must be an integer.\n',
            source=tool_name,
        )

    if distance_param > SPLICEAI_MAX_DISTANCE_LIMIT:
        return error_response(
            f'Invalid "distance": "{distance_param}". The value must be < {SPLICEAI_MAX_DISTANCE_LIMIT}.\n',
            source=tool_name,
        )
    ...
    if mask_param not in ("0", "1"):
        return error_response(
            f'Invalid "mask" value: "{mask_param}". The value must be either "0" or "1". For example: {SPLICEAI_EXAMPLE}\n',
            source=tool_name,
        )
    ...
    if request.remote_addr not in DISABLE_LOGGING_FOR_IPS:
        ...
        print(
            f"{logging_prefix}: {request.remote_addr}: {variant} processing with hg={genome_version}, "
            f"distance={distance_param}, mask={mask_param}",
            flush=True,
        )

    # check REDIS cache before processing the variant
    results = get_splicing_scores_from_redis(
        tool_name, variant, genome_version, distance_param, mask_param
    )
    if not results:
        try:
            if tool_name == "spliceai":
                results = get_spliceai_scores(
                    variant, genome_version, distance_param, int(mask_param)
                )
            elif tool_name == "pangolin":
                pangolin_mask_param = "True" if mask_param == "1" else "False"
                results = get_pangolin_scores(
                    variant, genome_version, distance_param, pangolin_mask_param
                )
            else:
                raise ValueError(f"Invalid tool_name: {tool_name}")
        except Exception as e:
        ...
        if "error" not in results:
            add_splicing_scores_to_redis(
                tool_name, variant, genome_version, distance_param, mask_param, results
            )
    ...
    if request.remote_addr not in DISABLE_LOGGING_FOR_IPS:
        print(
            f"{logging_prefix}: {request.remote_addr}: {variant} took {duration}",
            flush=True,
        )
...
def run_variant_liftover_tool(hg, chrom, pos, ref, alt, verbose=False):
    ...
    with tempfile.NamedTemporaryFile(
        suffix=".vcf", mode="wt", encoding="UTF-8"
    ) as input_file, tempfile.NamedTemporaryFile(
        suffix=".vcf", mode="rt", encoding="UTF-8"
    ) as output_file:
...
{chrom}	{pos}	.	{ref}	{alt}	60	."""
        ...
        try:
            ...
            if verbose:
                print(
                    f"{BCFTOOLS_LIFTOVER_TOOL} {hg} liftover on {chrom}:{pos} {ref}>{alt} returned: {results}",
                    flush=True,
                )
...
def run_UCSC_liftover_tool(hg, chrom, start, end, verbose=False):
    ...
    with tempfile.NamedTemporaryFile(
        suffix=".bed", mode="wt", encoding="UTF-8"
    ) as input_file, tempfile.NamedTemporaryFile(
        suffix=".bed", mode="rt", encoding="UTF-8"
        ...
        try:
            ...
            if verbose:
                print(
                    f"{UCSC_LIFTOVER_TOOL} {hg} liftover on {chrom}:{start}-{end} returned: {results}",
                    flush=True,
                )
            ...
            if len(result_fields) > 5:
                ...
                reason_liftover_failed = (
                    unmapped_output_file.readline().replace("#", "").strip()
                )
    ...
    if reason_liftover_failed:
        raise ValueError(
            f"{hg} liftover failed for {chrom}:{start}-{end} {reason_liftover_failed}"
        )
    else:
        raise ValueError(
            f"{hg} liftover failed for {chrom}:{start}-{end} for unknown reasons"
        )
...
def run_liftover():
    ...
    error_message = exceeds_rate_limit(
        request.remote_addr, request_type="liftover:total"
    )
    if error_message:
        print(
            f"{logging_prefix}: {request.remote_addr}: response: {error_message}",
    ...
    if not hg or hg not in VALID_HG_VALUES:
        return error_response(
            f'"hg" param error. It should be set to {" or ".join(VALID_HG_VALUES)}. For example: {LIFTOVER_EXAMPLE}\n'
        )
    ...
    if not format or format not in VALID_FORMAT_VALUES:
        return error_response(
            f'"format" param error. It should be set to {" or ".join(VALID_FORMAT_VALUES)}. For example: {LIFTOVER_EXAMPLE}\n'
        )
    ...
    if verbose:
        ...
        print(
            f"{logging_prefix}: {request.remote_addr}: {hg} liftover {format}: {chrom}:{variant_log_string}",
            flush=True,
        )
    ...
    if result and verbose:
        print(
            f"{hg} liftover on {variant_log_string} got results from cache: {result}",
            flush=True,
        )
    ...
    if not result:
        try:
            if format == "variant":
                result = run_variant_liftover_tool(
                    hg, chrom, pos, ref, alt, verbose=verbose
                )
            else:
...
def send_annotations(path):
    ...
    for filename in os.listdir("annotations"):
        ...
        last_modified = datetime.fromtimestamp(
            os.path.getmtime(os.path.join("annotations", filename))
        )
...
def _mimetype_for_path(rel_path):
...
def catch_all(path):
    ...
    if path in ("spliceai", "pangolin", "liftover"):
        return redirect(f"/{path}/", code=301)

    safe = os.path.normpath(os.path.join(APP_ROOT, path))
    if not safe.startswith(APP_ROOT):
        return Response("Not found", status=404)

    if os.path.isfile(safe):
        rel = os.path.relpath(safe, APP_ROOT)
        if rel.startswith(".."):
            return Response("Not found", status=404)
        return send_from_directory(APP_ROOT, rel, mimetype=_mimetype_for_path(rel))

    readme_path = os.path.join(APP_ROOT, "README.md")
    if os.path.isfile(readme_path):
        with open(readme_path, encoding="utf-8") as f:
            return Response(markdown2.markdown(f.read()), mimetype="text/html")
    return Response("Not found", status=404)
